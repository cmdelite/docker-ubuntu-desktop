...
RUN apt update -y && apt install --no-install-recommends -y xfce4 xfce4-goodies tigervnc-standalone-server novnc websockify sudo xterm init systemd snapd vim net-tools curl wget git tzdata dbus-x11 x11-utils x11-xserver-utils x11-apps software-properties-common gnupg python3
...
RUN curl -o /usr/bin/systemctl https://raw.githubusercontent.com/gdraheim/docker-systemctl-replacement/master/files/docker/systemctl.py && \
    sed -i 's|^#! /usr/bin/python|#!/usr/bin/python3|' /usr/bin/systemctl && \
